...
| Right-side icon tabs | Inspect and click Overview and By Level, then hover each tab. | Spyglass and book tabs form a vertical pair on the right edge; each has the correct tooltip and a clear selected state; no horizontal tab row or Settings tab appears. | PENDING | PENDING |
...
| Settings popup | Click the bottom-right gear, change units, minimap visibility, and diagnostics, close it, then reopen it. | A compact Settings popup opens above the footer gear; controls synchronize with persisted values and the popup toggles without changing the selected Overview/By Level tab. | PENDING | PENDING |
| Footer reset | Activate and cancel Reset Session. | The full Reset Session button is visible at bottom-left, the version and gear remain unobscured at bottom-right, and cancellation changes no data. | PENDING | PENDING |
...
| Footer and version | Inspect Overview and By Level at 80%, 100%, and 120% UI scale. | Reset Session, the Settings gear, and the version label are unobscured and do not overlap the center content or frame border. | PENDING | PENDING |
