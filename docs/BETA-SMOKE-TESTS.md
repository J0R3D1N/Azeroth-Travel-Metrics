...
| Focused ATM controls and layout regression | Inspect the title/minimap badge, native Condense and Expand controls, Settings footer, 10-pixel section gaps, layering, By Level scrollbar, and minimap rim placement. | Each focused control and layout behavior remains correct after the ATM rename. The native Aspect of the Cheetah icon appears in the title and minimap badge. | 2026-09-21: User confirmed the focused ATM smoke checklist passed before the native-icon migration; icon appearance requires retesting. | PARTIAL |
...
| Character panel shell | Open the regular panel and inspect the outer trim, title area, controls, and content background. | The shell has Public Library-style warm translucency; the gold title is correctly sized; the native Aspect of the Cheetah icon, minimize, close, and `ATM v1.0.0-beta` footer are visible; no transparent inner rectangle remains. | PENDING | PENDING |
...
| Right-side icon tabs | Inspect and click Overview and By Level, then hover each tab. | Overview uses the native spyglass icon and By Level uses the native book icon; both have the correct tooltip and a clear selected state. | PENDING | PENDING |
...
| Native travel minimap launcher | Inspect the minimap launcher, click it from closed and minimized states, and drag it to a new position. | The native Aspect of the Cheetah icon renders; clicks always open the regular panel; dragging and persisted positioning remain correct. | PENDING | PENDING |
| Minimap launcher outside rim | At 80%, 100%, and 120% UI scale, inspect the launcher at several saved angles, then drag it around the full perimeter on round, square, and available corner/side minimap shapes. | The Aspect of the Cheetah icon itself clears the actual minimap rim and renders above its border at every angle, shape, and scale; dragging preserves the cursor's polar angle and the saved position survives `/reload`. | PENDING | PENDING |
