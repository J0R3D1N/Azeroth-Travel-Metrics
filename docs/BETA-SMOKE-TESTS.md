...
- **Tester/date:** User-confirmed focused ATM release smoke test, 2026-09-21
...
| ATM rename, launch, and package identity | Install and launch the packaged ATM beta with the migrated SavedVariables, inspect the AddOns entry, title, footer, commands, and chat output. | AddOns lists **Azeroth Travel Metrics - WoW: Forever (beta)** version `1.0.0-beta`; the title is **Azeroth Travel Metrics**; the footer is `ATM v1.0.0-beta`; `/atm` and its subcommands work; legacy slash aliases are not registered; no legacy chat prefix or Lua error appears. | 2026-09-21: User confirmed the focused ATM smoke checklist passed. | PASS |
| Focused ATM controls and layout regression | Inspect the title/minimap badge, explicit minimize and close controls, Settings tab, section gaps, layering, By Level scrollbar, and minimap rim placement. | Each focused control and layout behavior remains correct after the redesign. The native pocket-watch icon appears in the portrait and minimap badge. | Previous layout passed before the spellbook redesign; the corrected candidate requires retesting. | PENDING |
| Migrated data close/reopen and `/reload` | Confirm the migrated Lifetime, This Session, and By Level values, close and reopen the panel, then run `/reload` and inspect the values again. | Migrated data remains intact across close/reopen and `/reload`, with no reset or corruption. | 2026-09-21: User confirmed the focused ATM smoke checklist passed and the migrated data remained intact. | PASS |
| Login/logout and `/reload` | Record totals, run `/reload`, then log out and back in; also cross a zone boundary without reloading. | `/reload` preserves This Session, Lifetime, By Level totals, and settings without adding a bridge segment. A full character login starts a fresh This Session while preserving Lifetime, By Level totals, and settings. Repeated zoning does not reset the session. | 2026-09-21: Full Windows client restart succeeded after applying ForeverSVFix v1.0.2; the addon loaded with the restored Lifetime and By Level data intact. The user later confirmed the focused ATM `/reload` persistence check passed. Session-reset, full relog, and zoning checks remain pending. | PARTIAL |
...
| Hybrid portrait window shell | Open the regular panel and inspect the outer trim, title area, controls, and content background. | A circular pocket-watch portrait and thin native title bar remain; native minimize and close are both visible inside the title bar and work; `ATM v1.0.0-beta` remains visible; no parchment artwork appears. | 2026-09-23: The parchment candidates were rejected; this candidate restores the last-known-good center inside the retained portrait shell. | PENDING |
...
| Right-side icon tabs | Inspect and click Overview, By Level, and Settings, then hover each tab. | Spyglass, book, and gear tabs form a matching vertical stack on the right edge; each has the correct tooltip and a clear selected state; no horizontal tab row appears. | PENDING | PENDING |
| Classic metric center | Inspect Overview and By Level at 80%, 100%, and 120% UI scale. | The last-known-good non-parchment center is restored: centered character-stat headers, dark rows, gold labels, white values, and no parchment page or spellbook divider. | PENDING | PENDING |
| Settings center | Select the right-side gear tab, change units, minimap visibility, and diagnostics, switch away, then return. | Settings replaces the center content using the same character-stat styling as the other views; controls synchronize with persisted values; enabled diagnostic reason/count output appears in its scrollable section. | PENDING | PENDING |
| Footer reset | Activate and cancel Reset Session. | The full Reset Session button is visible at bottom-left, the version remains unobscured at bottom-right, and cancellation changes no data. | PENDING | PENDING |
...
| Footer and version | Inspect Overview and By Level at 80%, 100%, and 120% UI scale. | Reset Session and the version label are unobscured, sit directly below the center content without a large blank region, and do not overlap the frame border. | PENDING | PENDING |
...
| HUD persistence and controls | Drag the HUD, `/reload`, hover it, use Restore and Close, minimize again, then click the minimap launcher. | Position persists; hover reveals equally sized working Restore and Close controls; minimap click hides the HUD and opens the regular panel. | PENDING | PENDING |
...
| Native travel minimap launcher | Inspect the minimap launcher, click it from closed and minimized states, and drag it to a new position. | The native pocket-watch icon renders; clicks always open the regular panel; dragging and persisted positioning remain correct. | PENDING | PENDING |
| Minimap launcher outside rim | At 80%, 100%, and 120% UI scale, inspect the launcher at several saved angles, then drag it around the full perimeter on round, square, and available corner/side minimap shapes. | The pocket-watch icon itself clears the actual minimap rim and renders above its border at every angle, shape, and scale; dragging preserves the cursor's polar angle and the saved position survives `/reload`. | PENDING | PENDING |
