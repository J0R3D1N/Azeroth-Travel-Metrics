...
| Character panel shell | Open the regular panel and inspect the outer trim, title bar, controls, and content inset. | The shell has bronze trim and an opaque brown interior with no game scene showing through; no portrait covers the upper-left ornament; minimize and settings sit fully inside and above the border. | PENDING | PENDING |
