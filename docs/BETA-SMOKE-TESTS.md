...
| Login/logout and `/reload` | Record totals, log out and back in, then run `/reload`; also cross a zone boundary without reloading. | Every new addon runtime starts a fresh This Session without adding a bridge segment; repeated zoning in the same runtime does not reset the session; Lifetime, By Level totals, and settings persist. | PENDING | PENDING |
...
| Character panel shell | Open the regular panel and inspect the outer trim, title area, controls, and content background. | The shell has Public Library-style warm translucency; the gold title is correctly sized; the Sprint icon, minimize, close, and version are visible; no transparent inner rectangle remains. | PENDING | PENDING |
| Close and reopen data preservation | Open the regular panel on a character with nonzero Lifetime, This Session, and Current Level values, record all visible tables and tracker state, close it, then reopen it from the minimap launcher or `/att`. | The same character, Lifetime table, This Session table, Current Level table, displayed values, and active tracker/runtime state remain unchanged. | PENDING | PENDING |
