...
| Spellbook window shell | Open the regular panel and inspect the outer trim, title area, controls, and content background. | A circular pocket-watch portrait appears at upper left; the native title bar is thinner than the prior custom bar; explicit minimize, close, and `ATM v1.0.0-beta` remain visible; parchment fills the content page without gaps or clipping. | PENDING | PENDING |
...
| Top interior icon tabs | Inspect and click Overview, By Level, and Settings, then hover each tab. | The three tabs form a left-justified horizontal row inside the upper window; spyglass, book, and gear artwork fills each button without crossing its border; each tab has the correct tooltip and a clear selected state. | PENDING | PENDING |
| Parchment and spellbook dividers | Inspect Overview and By Level at 80%, 100%, and 120% UI scale. | The parchment page covers the content area; Lifetime begins below the decorated page edge with no dark seam; every section has a visible spellbook divider; dark labels and values remain readable without overlap or clipping. | PENDING | PENDING |
| Settings parchment page | Open Settings, change units, minimap visibility, and diagnostics, switch tabs, then return to Settings. | Settings renders as the third parchment page; controls synchronize with persisted values and continue to update the same settings without a floating popup. | PENDING | PENDING |
| Settings reset danger zone | Open Settings and inspect Reset Current Session, then activate and cancel its confirmation. | The reset warning and full button are unobscured inside Settings; the footer remains clear; cancellation changes no data. | PENDING | PENDING |
...
| Footer and version | Inspect every tab at 80%, 100%, and 120% UI scale. | The footer contains only the unobscured version label; no Settings or Reset control overlaps the parchment or frame border. | PENDING | PENDING |
