...
| Spellbook window shell | Open the regular panel and inspect the outer trim, title area, controls, and content background. | A circular pocket-watch portrait appears at upper left; the native title bar is thinner than the prior custom bar; minimize, close, and `ATM v1.0.0-beta` remain visible; parchment fills the content page without gaps or clipping. | PENDING | PENDING |
...
| Top interior icon tabs | Inspect and click Overview and By Level, then hover each tab. | The tabs sit horizontally inside the upper window; Overview uses the native spyglass icon and By Level uses the native book icon; both have the correct tooltip and a clear selected state. | PENDING | PENDING |
| Parchment and spellbook dividers | Inspect Overview and By Level at 80%, 100%, and 120% UI scale. | The parchment page covers the content area; each section has a visible spellbook divider; labels and values remain readable without overlap, clipping, or transparent seams. | PENDING | PENDING |
